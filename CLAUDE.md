...
- **E2E tests** (`tests/e2e/`): Playwright serial mode. Requires Docker Supabase running. Global setup cleans test data (prefix `E2E*`, `Test Klijent*`).
- **`.env.test`** is auto-generated by `test:setup` with local Supabase keys. Never committed (in `.gitignore`).
- **`.env.local`** points to PRODUCTION — never use it for testing.
