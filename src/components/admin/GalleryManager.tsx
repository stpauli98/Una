...
export function GalleryManager({
  ...
  // activeCategory je sada ISKLJUČIVO meta za novi upload (ne filter mreže).
  ...
  // Kartica jedne slike (DRY — koristi se u svakoj kategorijskoj sekciji).
  const imageCard = (item: GalleryItem) => {
    ...
    return (
      <div
        ...
        className={cn(
          ...
          isSelected ? "border-rose ring-2 ring-rose/30" : "border-cream",
        )}
      >
        <Image
          src={item.url}
          ...
          className={cn("object-cover", isSelected && "opacity-75")}
        />
        ...
        {!selectMode && (
          <button
            ...
            disabled={pending}
            onClick={() => {
              if (!confirm("Obrisati ovu sliku? Ne može se vratiti.")) return;
              startTransition(async () => {
    ...
    <>
      ...
      <div>
        ...
        <div className="mb-8">
          ...
          {/* Meta za upload — biraš u koju kategoriju ide nova slika */}
          {categories.length > 0 && (
            <div className="mb-3 flex flex-wrap items-center gap-2">
              <label
                htmlFor="upload-category"
                className="text-[11px] uppercase tracking-wider text-light"
              >
                Učitaj u kategoriju:
              </label>
              <select
                ...
                className="border border-cream bg-white px-3 py-1.5 text-[12px] text-dark focus:border-rose focus:outline-none disabled:opacity-50"
              >
                {CATEGORIES.map((cat) => (
                    ...
                    <div className="relative aspect-square overflow-hidden border border-cream bg-marble">
                      ...
                      {pending && (
                        <div className="absolute inset-0 flex items-center justify-center">
                          <Loader2 size={18} className="animate-spin text-rose" />
                        </div>
                      )}
                      {!pending && (
                        <button
        ...
        {/* Selection toolbar — radi nad SVIM slikama */}
        {items.length > 0 && (
        ...
        {/* Galerija — sve kategorije vidljive, grupisane */}
        {items.length === 0 ? (
          <div className="border border-cream bg-white p-10 text-center">
            <p className="text-sm text-light">Još nema slika.</p>
            <p className="mt-1 text-[11px] text-light">
              Izaberite kategoriju i učitajte slike iznad.
            </p>
          </div>
        ) : (
          <div className="space-y-8">
            {CATEGORIES.map((cat) => {
              const catImages = items.filter((i) => i.category === cat.key);
              return (
                <div key={cat.key}>
                  <h3 className="mb-3 font-display text-lg text-dark">
                    {cat.label}{" "}
                  ...
                  {catImages.length === 0 ? (
                    <p className="border border-cream bg-white p-4 text-center text-[12px] text-light">
                      Nema slika u ovoj kategoriji.
                    </p>
                  ) : (
                    <div className="grid grid-cols-2 gap-3 md:grid-cols-3 lg:grid-cols-4">
                      {catImages.map((item) => imageCard(item))}
                    </div>
                  )}
                </div>
              );
